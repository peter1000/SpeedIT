...
size_list = len(test_list1)


# define SpeedIT functions
def example_access_normal_for():
   for item in test_list1:
      temp = item


def example_access_by_idx_normal_for():
   for item in range(size_list):
      temp = test_list1[item]


def example_access_reversed_for():
   for item in reversed(test_list1):
      temp = item


def example_access_reversed_by_idx_normal_for():
   for item in range(size_list - 1, -1, -1):
      temp = test_list1[item]
...
def main():
   ...
   func_dict = {
      ...
      'example_access_normal_for': (example_access_normal_for, [], {}),
      'example_access_by_idx_normal_for': (example_access_by_idx_normal_for, [], {}),
      'example_access_reversed_for': (example_access_reversed_for, [], {}),
      'example_access_reversed_by_idx_normal_for': (example_access_reversed_by_idx_normal_for, [], {}),
   }

   setup_line_list = [
      'from collections import deque',
      'from __main__ import test_list1, size_list'
   ]

   check_run_sec = 1
   with open('result_output/Example16ListAccessItems.txt', 'w') as file_:
      file_.write('\n\n Example16ListAccessItems.py output\n\n')
